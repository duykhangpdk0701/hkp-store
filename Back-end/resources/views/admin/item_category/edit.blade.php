...
@section('content')
    <div id="basic" class="col-lg-12 col-sm-12 col-12 layout-spacing">
        <form action="{{ route('admin.item_category.update', $itemCategory->id) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="statbox widget box box-shadow">
                <div class="widget-content widget-content-area">
                    ...
                    @foreach (config('app.locales') as $locale)
                        <div class="form-group mb-4">
                            ...
                            <input type="text" name="name[{{ $locale }}]"
                                ...
                                value="{{ old('name.' . $locale, $itemCategory->getTranslation('name', $locale)) }}">
                            @error('name.' . $locale)
                    ...
                    @endforeach
                    <div class="form-group mb-4">
                        <label for="sRolePicker">{{ __('general.common.parent_category') }}</label>
                        <div>
                            <select class="selectpicker w-100 @error('parent_id') is-invalid @enderror" id="sRolePicker"
                                ...
                                <option value="" selected>{{ __('general.common.no_parent') }}</option>
                    ...
                    <div class="form-group mb-4">
                        ...
                        <textarea class="form-control @error('description') is-invalid @enderror" id="textarea" rows="3"
                            ...
                            name="description">{{ old('description') ? old('description') : $itemCategory->description }}</textarea>
                        @error('description')
                            <span class="invalid-feedback" role="alert">
                                <strong>{{ $message }}</strong>
                            </span>
                    ...
                    <div class="form-group mb-4">
                        ...
                        <input type="number" name="order" class="form-control @error('order') is-invalid @enderror"
                            ...
                            value="{{ old('order') ? old('order') : $itemCategory->order }}">
                        @error('order')
                            <span class="invalid-feedback" role="alert">
                                <strong>{{ $message }}</strong>
                            </span>
...
@push('script')
    <script>
        $('.selectpicker').selectpicker('val', '{{ @old('parent_id') ? @old('parent_id') : $itemCategory->parent_id }}');
    </script>
@endpush
